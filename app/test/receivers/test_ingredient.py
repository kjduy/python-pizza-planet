...
def test_get_all(app, ingredients: list):
    created_ingredients = list(map(lambda ingredient: Invoker(CreateCommand(IngredientReceiver, ingredient)).execute()[0], ingredients))
...
def test_get_by_id(app, ingredient: dict):
    ...
    invoker_to_get_by_id = Invoker(GetByIdCommand(IngredientReceiver, created_ingredient['_id']))
...
def test_update(app, ingredient: dict):
    ...
    invoker_to_update = Invoker(UpdateCommand(IngredientReceiver, {'_id': created_ingredient['_id'], **updated_fields}))
