...
def test_get_all(app, sizes: list):
    created_sizes = list(map(lambda size: Invoker(CreateCommand(SizeReceiver, size)).execute()[0], sizes))
...
def test_update(app, size: dict):
    ...
    invoker_to_update = Invoker(UpdateCommand(SizeReceiver, {'_id': created_size['_id'], **updated_fields}))
