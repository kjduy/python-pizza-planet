...
def test_get_all(app, beverages: list):
    created_beverages = list(
        map(
            lambda beverage: Invoker(
                CreateCommand(BeverageReceiver, beverage)
            ).execute()[0],
            beverages,
        )
    )
...
def test_get_by_id(app, beverage: dict):
    ...
    invoker_to_get_by_id = Invoker(
        GetByIdCommand(BeverageReceiver, created_beverage["_id"])
    )
...
def test_update(app, beverage: dict):
    ...
    invoker_to_update = Invoker(
        UpdateCommand(
            BeverageReceiver, {"_id": created_beverage["_id"], **updated_fields}
        )
    )
