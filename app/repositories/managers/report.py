...
class ReportManager(BaseManager):
    ...
    def get_most_requested_ingredient(cls):
        most_requested_ingredient = (
            ...
            .order_by(text("num_requested DESC"))
            .first()
        )

        return {
            "name": most_requested_ingredient.name,
            "num_requested": most_requested_ingredient.num_requested,
        }
    ...
    def get_month_with_more_revenue(cls):
        ...
        month_name = datetime.strptime(month_with_more_revenue.month, "%m").strftime(
            "%B"
        )

        return {
            "month": month_name,
            "revenue": round(month_with_more_revenue.revenue, 2),
        }
